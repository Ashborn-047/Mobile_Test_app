...
export const MindMesh = () => {
  ...
  const [meditationHistory, setMeditationHistory] = useState<
    MeditationSession[]
  >([]); // New state for meditation history
  ...
  useEffect(() => {
    ...
    const savedMeditationHistory =
      storage.get<MeditationSession[]>("meditationHistory"); // Load meditation history
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const personalityProfile = storage.get<PersonalityProfileV2>(
      ...
      const updatedMeditationHistory = [
        ...meditationHistory,
  ...
  return (
    <div className="min-h-screen p-6 max-w-4xl mx-auto relative overflow-hidden">
      ...
      >
        <div className="bg-gradient-to-r from-teal-light to-turquoise/70 rounded-2xl border border-turquoise/30 px-6 py-7 text-center shadow-[0_0_30px_rgba(9,218,198,0.08)] mb-8">
          ...
          <p className="mt-2 text-[16px] text-teal-dark/80">
            Find your center with AI-guided mindfulness
          </p>
        </div>
          ...
          >
            <Card className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-turquoise/70 text-sm">
                  {result.duration} min
                </span>
              </div>
              <p className="text-turquoise/90 text-lg leading-relaxed mb-4">
                {result.text}
              </p>
          ...
          >
            <Card>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-turquoise/70 mb-2">
                    How are you feeling?
                  </label>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["Relax", "Focus", "Motivate", "Sleep", "Clarity"].map(
                      (g) => (
                        <Button
                          key={g}
                          type="button"
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, #09dac6 0%, #09dac6 ${((duration - 1) / 59) * 100}%, #0a4d42 ${((duration - 1) / 59) * 100}%, #0a4d42 100%)`,
                    }}
                  />
                </div>
                <Button
      ...
      >
        ...
        <Card className={`${gradients.mindMeshCard} p-6`}>
          <p className="text-white/80">
            "Based on your personality, you might benefit from grounding
            exercises when feeling overwhelmed."
          </p>
          <Button
            variant="secondary"
            className="mt-4"
        ...
        <Card className={`${gradients.mindMeshCard} p-6`}>
          {moodHistory.length === 0 ? (
            <p className="text-white/70">
              No mood entries yet. Start a session!
            </p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {moodHistory.map((entry) => (
                <motion.div
                  ...
                  className="bg-teal-light/50 p-3 rounded-lg border border-turquoise/20"
                >
      ...
      >
        <h2 className="text-3xl font-bold text-dark-teal mb-6">
          Meditation History 🧘‍♀️
        </h2>
        <Card className={`${gradients.mindMeshCard} p-6`}>
          {meditationHistory.length === 0 ? (
            <p className="text-white/70">
              No meditation sessions yet. Generate your first one!
            </p>
          ) : (
            <div className="space-y-4">
              {meditationHistory.map((session) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3 }}
                  className="bg-teal-light/50 p-4 rounded-lg border border-turquoise/20 text-left"
                >
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 bg-turquoise/20 text-turquoise rounded-full text-xs font-medium">
                  ...
                  <p className="text-white/60 text-xs">
                    Mood Before: {session.moodBefore} |{" "}
