import { PersonalityTrait, PersonalityProfileV2, ComparisonResult, QuizQuestion } from './types';

export const calculateRadarData = (profile: PersonalityProfileV2): PersonalityTrait[] => {
...
export const getTraitChange = (before: number, after: number): 'up' | 'down' | 'same' => {
...
export const comparePersonalityProfiles = (
  ...
  profileBefore.traits.forEach((traitBefore) => {
    const traitAfter = profileAfter.traits.find((t) => t.trait === traitBefore.trait);
    if (traitAfter) {
      comparison.push({
...
export const generatePersonalitySummary = (answers: Record<string, number>): { summary: string; traits: PersonalityTrait[]; emoji: string } => {
  // Mock AI summary based on answers
  const traitsMap: Record<string, number> = {};
  for (const question of personalityQuestions) {
    const answerValue = answers[question.id];
    if (answerValue) {
      traitsMap[question.category] = (traitsMap[question.category] || 0) + answerValue;
    }
  }

  const traits: PersonalityTrait[] = Object.entries(traitsMap).map(([trait, value]) => ({
    trait,
    value: Math.round(value * 10), // Scale up for radar chart
  }));

  const summary = "Based on your responses, you exhibit a balanced profile with strong tendencies towards openness and conscientiousness. You are likely a thoughtful and organized individual who enjoys new experiences and pays attention to detail.";
