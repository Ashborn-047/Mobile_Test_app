...
const mockReadingList: Book[] = [
  ...
  },
  {
    ...
    link: "#",
    coverImage:
      "https://via.placeholder.com/150/09dac6/05352d?text=Lean+Startup",
  },
  {
    id: "3",
  ...
  },
  {
    ...
    link: "#",
    coverImage:
      "https://via.placeholder.com/150/09dac6/05352d?text=Zero+to+One",
  },
  {
    id: "5",
    title: "Deep Work",
...
export const CareerReadingList: React.FC = () => {
  const renderBookCard = (book: Book) => (
    <Card
      key={book.id}
      className="p-4 bg-gray-50 border-gray-200 text-dark-teal flex items-center space-x-4"
    >
      <img
        src={book.coverImage}
        alt={book.title}
      ...
      <div>
        ...
        <a
          ...
          className="text-turquoise text-xs mt-2 inline-block hover:underline"
        >
          View Book →
        </a>
      </div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mockReadingList
            .filter((book) => book.category === "MUST_READ_FIRST")
            .map(renderBookCard)}
        </div>
      </div>
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mockReadingList
            .filter((book) => book.category === "READ_NEXT")
            .map(renderBookCard)}
        </div>
      </div>
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mockReadingList
            .filter((book) => book.category === "READ_EVENTUALLY")
            .map(renderBookCard)}
        </div>
      </div>
      <div className="text-center">
        <Button
          variant="secondary"
          onClick={() =>
            alert("Coming soon: Personalized Book Recommendations!")
          }
        >
