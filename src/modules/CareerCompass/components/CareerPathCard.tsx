...
export const CareerPathCard = ({ career }: CareerPathCardProps) => {
  ...
  const [tasks, setTasks] = useState(() => {
    ...
    const storedTasks = storage.get<Record<string, boolean>>(
      `career_path_${career.id}_tasks`,
    );
    return (
      storedTasks || {
        pick_problem: false,
  ...
  const handleStartPath = () => {
    storage.set("activeCareerPath", {
      ...career,
      startedAt: new Date().toISOString(),
    });
    alert(
      `Started path: ${career.title}! Track your progress in the Dashboard.`,
    );
  };
  ...
  return (
    <Card
      className={`${gradients.careerCard} col-span-1 h-auto flex flex-col justify-between p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300`}
    >
      <div
      ...
      >
        ...
        <div className="mb-4">
          ...
          <div className="w-full h-32 bg-gray-700 rounded-lg mb-4 flex items-center justify-center text-white text-sm">
            Hero Illustration Placeholder
          </div>
        ...
        {isExpanded && (
          ...
          >
            ...
            <p className="text-white/90 text-sm mb-4">
              "Right-brain entrepreneurs innovate where left-brain ones
              optimize. You create products people LOVE, not just products that
              work."
            </p>
            ...
            <div className="space-y-2 mb-4">
              {renderTaskCheckbox(
                "pick_problem",
                "Pick ONE problem you're passionate about",
                "This Week",
              )}
              {renderTaskCheckbox(
                "build_mvp",
            ...
            <Button onClick={handleStartPath} className="w-full mt-4">
              START THIS PATH
            </Button>
          </motion.div>
        )}
