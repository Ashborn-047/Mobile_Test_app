...
export const BudgetCSVImport: React.FC<BudgetCSVImportProps> = ({ onImport }) => {
  ...
  const parseCSV = (text: string) => {
    ...
    dataLines.forEach((line, index) => {
      ...
      const amountIndex = columns.findIndex(col => !isNaN(parseFloat(col.replace(/[^\d.-]/g, ''))));
      if (amountIndex !== -1) {
      ...
      const dateIndex = columns.findIndex(col => !isNaN(new Date(col).getTime()));
      if (dateIndex !== -1) {
      ...
      if (isNaN(amount) || amount === 0) {
        console.warn(`Skipping line ${index + 1} due to invalid amount: ${line}`);
  ...
  return (
    <Card className="p-6 bg-teal-light/50 border border-turquoise/30 rounded-2xl shadow-lg">
      <h3 className="text-xl font-semibold text-white mb-4">📄 Import Expenses from CSV</h3>
      <div className="mb-4">
        <input
          type="file"
          accept=".csv"
      ...
      <Button onClick={handleProcessCsv} disabled={!csvFile} className="w-full mb-4">
        Preview CSV
      </Button>
      ...
      {previewData.length > 0 && (
        ...
        >
          <h4 className="text-lg font-semibold text-white mb-3">Preview ({previewData.length} items)</h4>
            ...
            <table className="min-w-full text-sm text-white/90">
              ...
              <tbody>
                {previewData.map((expense) => (
                  <tr key={expense.id} className="odd:bg-teal-light/20 even:bg-teal-light/10">
          ...
          <Button onClick={handleConfirmImport} className="w-full">Confirm Import</Button>
        </motion.div>
      )}
    </Card>
  );
