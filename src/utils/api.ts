...
export const api = {
  ...
  async submitPersonalityQuiz(): Promise<PersonalityProfileV2> {
    await delay(1500);
    // This is now handled by generatePersonalitySummary in Personality/utils.ts
    // and Personality/components/Results.tsx
    throw new Error(
      "This method is deprecated. Personality quiz logic moved to client-side utilities.",
    );
  },
  ...
  async getCareerRecommendations(
    ...
    if (profile) {
      const opennessTrait = profile.traits.find(
        (t: PersonalityTrait) => t.trait === "Openness",
      );
      const conscientiousnessTrait = profile.traits.find(
        (t: PersonalityTrait) => t.trait === "Conscientiousness",
      );
      const extraversionTrait = profile.traits.find(
        (t: PersonalityTrait) => t.trait === "Extraversion",
      );
  ...
  async generateMeditation(
    ...
    _goal: string,
    duration: number,
    personalityProfile: PersonalityProfileV2 | null,
  ): Promise<Meditation> {
    await delay(1200);
    ...
    const isAffirmation =
      ...
      mood.toLowerCase().includes("stressed");

    let personalityInsight = "";
    if (personalityProfile) {
      const topTrait = personalityProfile.traits.sort(
        (a: PersonalityTrait, b: PersonalityTrait) => b.value - a.value,
      )[0];
      if (topTrait) {
